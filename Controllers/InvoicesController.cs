using Fiscalapi.Abstractions;
using Fiscalapi.Models;
using Fiscalapi.Services;
using Microsoft.AspNetCore.Mvc;

// For more information on enabling Web API for empty projects, visit https://go.microsoft.com/fwlink/?LinkID=397860

namespace FiscalApi.Samples.AspNet.Controllers
{
    [Route("api/[controller]")]
    [ApiController]
    public class InvoicesController : ControllerBase
    {
        private readonly IFiscalApiClient _fiscalapiClient;

        public InvoicesController(IFiscalApiClient fiscalapiClient)
        {
            _fiscalapiClient = fiscalapiClient;
        }


        // Crear una factura por valores (enviando todos los datos de la solicitud)
        [HttpPost("factura-ingreso-por-valores")]
        public async Task<IActionResult> FacturaIngresoPorValores(CancellationToken ct = default)
        {
            // Crear factura por valores


            // Crear certificados de prueba (EKU9003173C9)
            var sellos = new List<TaxCredential>()
            {
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.CertificateCsd,
                    Password = "12345678a"
                },
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.PrivateKeyCsd,
                    Password = "12345678a"
                }
            };

            // Emisor
            var issuer = new InvoiceIssuer
            {
                Tin = "EKU9003173C9",
                LegalName = "ESCUELA KEMPER URGATE",
                TaxRegimeCode = "601",
                TaxCredentials = sellos
            };

            // Receptor
            var recipient = new InvoiceRecipient
            {
                Tin = "EKU9003173C9",
                LegalName = "ESCUELA KEMPER URGATE",
                ZipCode = "42501",
                TaxRegimeCode = "601",
                CfdiUseCode = "G01",
                Email = "someone@somewhere.com"
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    ItemCode = "01010101",
                    Quantity = 9.5m,
                    UnitOfMeasurementCode = "E48",
                    UnitOfMeasurement = "Unidad de servicio",
                    Description = "Invoicing software as a service",
                    UnitPrice = 3587.75m,
                    TaxObjectCode = "02",
                    ItemSku = "7506022301697",
                    Discount = 255.85m,
                    ItemTaxes = new List<InvoiceItemTax>()
                    {
                        new InvoiceItemTax
                        {
                            TaxCode = "002", // IVA
                            TaxTypeCode = "Tasa", // Tasa
                            TaxRate = 0.160000m, // 16%
                            TaxFlagCode = "T" // Traslado
                        }
                    }
                }
            };

            // Crear la factura 
            var invoice = new Invoice
            {
                VersionCode = "4.0",
                Series = "SDK-F",
                Date = DateTime.Now,
                PaymentFormCode = "01",
                CurrencyCode = "MXN",
                TypeCode = "I",
                ExpeditionZipCode = "42501",
                Issuer = issuer,
                Recipient = recipient,
                Items = items,
                PaymentMethodCode = "PUE",
            };


            // Timbrar la factura
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(invoice);


            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);
                Console.WriteLine($@"Factura {apiResponse.Data.Number} creada");
            }
            else
            {
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }

            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }


        // Crear nota de credito por valores (enviando todos los datos de la solicitud)
        [HttpPost("nota-credito-por-valores")]
        public async Task<IActionResult> NotaCreditoPorValores(CancellationToken ct = default)
        {
            //Crear factura de egreso (nota de crédito) por valores


            // Crear certificados
            var taxCredentials = new List<TaxCredential>()
            {
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.CertificateCsd,
                    Password = "12345678a"
                },
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.PrivateKeyCsd,
                    Password = "12345678a"
                }
            };

            // Crear un nuevo emisor
            var issuer = new InvoiceIssuer
            {
                Tin = "FUNK671228PH6",
                LegalName = "KARLA FUENTE NOLASCO",
                TaxRegimeCode = "621",
                TaxCredentials = taxCredentials
            };

            // Crear un nuevo receptor
            var recipient = new InvoiceRecipient
            {
                Tin = "EKU9003173C9",
                LegalName = "ESCUELA KEMPER URGATE",
                ZipCode = "42501",
                TaxRegimeCode = "601",
                CfdiUseCode = "G01",
                Email = "someone@somewhere.com"
            };

            // Agregar facturas relacionadas
            var relatedInvoices = new List<RelatedInvoice>()
            {
                new RelatedInvoice
                {
                    Uuid = "5FB2822E-396D-4725-8521-CDC4BDD20CCF",
                    RelationshipTypeCode = "01"
                }
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    ItemCode = "01010101",
                    Quantity = 0.5m,
                    UnitOfMeasurementCode = "E48",
                    Description = "Invoicing software as a service",
                    UnitPrice = 3587.75m,
                    TaxObjectCode = "02",
                    ItemSku = "7506022301697",
                    ItemTaxes = new List<InvoiceItemTax>()
                    {
                        new InvoiceItemTax
                        {
                            TaxCode = "002",
                            TaxTypeCode = "Tasa",
                            TaxRate = 0.160000m,
                            TaxFlagCode = "T"
                        }
                    }
                }
            };

            // Crear la factura de egreso (nota de crédito)
            var creditNote = new Invoice
            {
                VersionCode = "4.0",
                Series = "CN",
                Date = DateTime.Now,
                PaymentFormCode = "03",
                PaymentConditions = "Contado",
                CurrencyCode = "MXN",
                TypeCode = "E", // E para egreso (nota de crédito)
                ExpeditionZipCode = "01160",
                Issuer = issuer,
                Recipient = recipient,
                Items = items,
                PaymentMethodCode = "PUE",
                ExchangeRate = 1,
                ExportCode = "01",
                RelatedInvoices = relatedInvoices
            };

            // Timbrar la factura de egreso
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(creditNote);

            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);

                Console.WriteLine($@"Nota de Crédito {apiResponse.Data.Number} creada exitosamente");
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }


            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }


        // Crear complemento pago por valores (enviando todos los datos de la solicitud)
        [HttpPost("complemento-pago-por-valores")]
        public async Task<IActionResult> ComplementoPagoPorValores(CancellationToken ct = default)
        {
            //Crear factura de pago (complemento pago) por valores


            // Crear certificados (usando los mismos del ejemplo anterior)
            var taxCredentials = new List<TaxCredential>()
            {
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.CertificateCsd,
                    Password = "12345678a"
                },
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.PrivateKeyCsd,
                    Password = "12345678a"
                }
            };

            // Crear un nuevo emisor
            var issuer = new InvoiceIssuer
            {
                Tin = "FUNK671228PH6",
                LegalName = "KARLA FUENTE NOLASCO",
                TaxRegimeCode = "621",
                TaxCredentials = taxCredentials
            };

            // Crear un nuevo receptor
            var recipient = new InvoiceRecipient
            {
                Tin = "EKU9003173C9",
                LegalName = "ESCUELA KEMPER URGATE",
                ZipCode = "42501",
                TaxRegimeCode = "601",
                CfdiUseCode = "CP01",
                Email = "someone@somewhere.com"
            };

            // Crear una lista de productos o servicios de la factura (para pagos solo se usa un concepto genérico)
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    ItemCode = "84111506",
                    Quantity = 1m,
                    UnitOfMeasurementCode = "ACT",
                    Description = "Pago",
                    UnitPrice = 0m,
                    TaxObjectCode = "01"
                }
            };

            // Crear la lista de pagos


            var payments = new List<InvoicePayment>()
            {
                new InvoicePayment
                {
                    PaymentDate = DateTime.Now,
                    PaymentFormCode = "28",
                    CurrencyCode = "MXN",
                    Amount = 28599.99m,
                    SourceBankTin = "BSM970519DU8",
                    SourceBankAccount = "1234567891012131",
                    TargetBankTin = "BBA830831LJ2",
                    TargetBankAccount = "1234567890",
                    PaidInvoices = new List<PaidInvoice>
                    {
                        new PaidInvoice
                        {
                            Uuid = "5C7B0622-01B4-4EB8-96D0-E0DEBD89FF0F",
                            Series = "F",
                            Number = "1501",
                            CurrencyCode = "MXN",
                            PartialityNumber = 1,
                            SubTotal = 10000m,
                            PreviousBalance = 9533.33m,
                            PaymentAmount = 9533.33m,
                            RemainingBalance = 0m,
                            TaxObjectCode = "02",
                            PaidInvoiceTaxes = new List<PaidInvoiceTax>
                            {
                                new PaidInvoiceTax
                                {
                                    TaxCode = "002",
                                    TaxTypeCode = "Tasa",
                                    TaxRate = 0.160000m,
                                    TaxFlagCode = "T"
                                }
                            }
                        }
                    }
                }
            };


            // Crear la factura de pago
            var paymentInvoice = new Invoice
            {
                VersionCode = "4.0",
                Series = "CP",
                Date = DateTime.Now,
                ExchangeRate = 1,
                CurrencyCode = "XXX",
                TypeCode = "P", // P para pagos
                ExpeditionZipCode = "01160",
                Issuer = issuer,
                Recipient = recipient,
                Items = items,
                Payments = payments
            };

            // Timbrar la factura de pago
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(paymentInvoice);

            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);

                Console.WriteLine($@"Factura de Pago {apiResponse.Data.Number} creada exitosamente");
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }


            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }


        //Crear factura global por valores (enviando todos los datos de la solicitud)
        [HttpPost("factura-global-por-valores")]
        public async Task<IActionResult> FacturaGlobalPorValores(CancellationToken ct = default)
        {
            // https://docs.fiscalapi.com/credentials-info
          
            var sellos = new List<TaxCredential>()
            {
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.CertificateCsd,
                    Password = "12345678a"
                },
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.PrivateKeyCsd,
                    Password = "12345678a"
                }
            };

            // Emisor
            var issuer = new InvoiceIssuer
            {
                Tin = "FUNK671228PH6",
                LegalName = "KARLA FUENTE NOLASCO",
                TaxRegimeCode = "621",
                TaxCredentials = sellos
            };

            // Receptor
            var recipient = new InvoiceRecipient
            {
                Tin = "XAXX010101000",
                LegalName = "PUBLICO EN GENERAL",
                ZipCode = "01160",
                TaxRegimeCode = "616",
                CfdiUseCode = "S01",
                Email = "someone@somewhere.com"
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    ItemCode = "01010101",
                    Quantity = 1,
                    UnitOfMeasurementCode = "ACT",
                    Description = "Venta",
                    UnitPrice = 1230.00m,
                    TaxObjectCode = "02",
                    ItemSku = "venta0001",
                    Discount = 255.85m,
                    ItemTaxes = new List<InvoiceItemTax>()
                    {
                        new InvoiceItemTax
                        {
                            TaxCode = "002", // IVA
                            TaxTypeCode = "Tasa", // Tasa
                            TaxRate = 0.160000m, // 16%
                            TaxFlagCode = "T" // Traslado
                        }
                    }
                }
            };


            // Información global

            var globalInfo = new GlobalInformation
            {
                PeriodicityCode = "01",
                MonthCode = "05",
                Year = 2025
            };


            // Crear la factura 
            var invoice = new Invoice
            {
                VersionCode = "4.0",
                TypeCode = "I",
                Series = "F",
                Date = DateTime.Now,
                CurrencyCode = "MXN",
                PaymentFormCode = "01",
                PaymentMethodCode = "PUE",
                ExpeditionZipCode = "01160",
                GlobalInformation = globalInfo, // Información global
                Issuer = issuer, // Emisor
                Recipient = recipient, // Receptor
                Items = items, // Ventas 
            };


            // Timbrar la factura de pago
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(invoice);

            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);

                Console.WriteLine($@"Factura Global {apiResponse.Data.Number} creada exitosamente");
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }


            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }


        // Crear factura global por referencias (enviando solo los IDs)
        [HttpPost("factura-global-por-referencias")]
        public async Task<IActionResult> FacturaGlobalPorReferencias(CancellationToken ct = default)
        {
            // https://docs.fiscalapi.com/credentials-info
            
            // Emisor
            var issuer = new InvoiceIssuer
            {
                Id = "78d380fd-1b69-4e3c-8bc0-4f57737f7d5f"
            };

            // Receptor
            var recipient = new InvoiceRecipient
            {
                Id = "4e7ba2d7-2302-42f1-9fe4-6b75069f0fc9"
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    ItemCode = "01010101",
                    Quantity = 1,
                    UnitOfMeasurementCode = "ACT",
                    Description = "Venta",
                    UnitPrice = 1230.00m,
                    TaxObjectCode = "02",
                    ItemSku = "venta0001",
                    Discount = 255.85m,
                    ItemTaxes = new List<InvoiceItemTax>()
                    {
                        new InvoiceItemTax
                        {
                            TaxCode = "002", // IVA
                            TaxTypeCode = "Tasa", // Tasa
                            TaxRate = 0.160000m, // 16%
                            TaxFlagCode = "T" // Traslado
                        }
                    }
                }
            };


            // Información global

            var globalInfo = new GlobalInformation
            {
                PeriodicityCode = "01",
                MonthCode = "05",
                Year = 2025
            };


            // Crear la factura 
            var invoice = new Invoice
            {
                VersionCode = "4.0",
                TypeCode = "I",
                Series = "F",
                Date = DateTime.Now,
                CurrencyCode = "MXN",
                PaymentFormCode = "01",
                PaymentMethodCode = "PUE",
                ExpeditionZipCode = "01160",
                GlobalInformation = globalInfo, // Información global
                Issuer = issuer, // Emisor
                Recipient = recipient, // Receptor
                Items = items, // Ventas 
            };


            // Timbrar la factura de pago
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(invoice);

            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);

                Console.WriteLine($@"Factura Global {apiResponse.Data.Number} creada exitosamente");
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }


            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }


        // cancelar-factura pago por valores (enviando todos los datos de la solicitud)
        [HttpDelete("cancelar-factura-por-valores")]
        public async Task<IActionResult> CancelarFacturaPorValores(CancellationToken ct = default)
        {
            //Crear factura de pago (complemento pago) por valores


            //Certificados ESCUELA KEMPER URGATE
            var credentials = new List<TaxCredential>()
            {
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.CertificateCsd,
                    Password = "12345678a"
                },
                new TaxCredential
                {
                    Base64File =
                        "",
                    FileType = FileType.PrivateKeyCsd,
                    Password = "12345678a"
                }
            };


            var cancelRequest = new CancelInvoiceRequest()
            {
                InvoiceUuid = "c0d71795-dea8-40f6-9c30-affcf4c313f3", // Uuid de la factura (no el id de la misma)
                Rfc = "EKU9003173C9",
                CancellationReasonCode = "01",
                ReplacementUuid = "de841944-bd4f-4bb8-adfe-2a2282787c62",
                TaxCredentials = credentials
            };


            // Cancelar la factura
            var apiResponse = await _fiscalapiClient.Invoices.CancelAsync(cancelRequest);
            if (apiResponse.Succeeded)
            {
                Console.WriteLine("Factura cancelada exitosamente");
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }


            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }


        // Enviar factura por correo por valores (enviando todos los datos de la solicitud)
        [HttpPost("enviar-factura-email-por-valores")]
        public async Task<IActionResult> EnviarFacturaEmailPorValores(CancellationToken ct = default)
        {
            /* Enviar factura por email por valores*/

            // request model (personaliza el pdf de la factura)
            var request = new SendInvoiceRequest
            {
                InvoiceId = "1152fd7f-9470-4247-96e7-f1b3a28fdf6c",
                BandColor = "#FFA500",
                FontColor = "#FFFFFF",
                ToEmail = "mendoza.git@gmail.com", // email1@dominio;email2@dominio
                Base64Logo =
                    ""
            };

            // Send request
            var apiResponse = await _fiscalapiClient.Invoices.SendAsync(request);

            // Check response
            if (apiResponse.Succeeded)
            {
                Console.WriteLine("Factura enviada.");
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }

            if (apiResponse.Succeeded)
                return Ok(apiResponse);

            return BadRequest(apiResponse);
        }

        // Generar PDF por valores
        [HttpPost("generar-pdf-por-valores")]
        public async Task<IActionResult> GenerarPdfPorValores()
        {
            // request model
            var request = new CreatePdfRequest
            {
                InvoiceId = "51a3a6d3-b617-4a6b-8a67-cbfd654c7e90",
                BandColor = "#FFA500",
                FontColor = "#FFFFFF",
                Base64Logo =
                    ""
            };


            // Send request
            var apiResponse = await _fiscalapiClient.Invoices.GetPdfAsync(request);


            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }

        // Crear factura de ingreso por referencias (enviando solo lod ids de los objetos en la solicitud)
        [HttpPost("factura-ingreso-por-referencia")]
        public async Task<IActionResult> CrearFacturaPorReferencia(CancellationToken ct = default)
        {
            /* Crear factura por referencia*/

            // Emisor KARLA FUENTE NOLASCO
            var issuer = new InvoiceIssuer
            {
                Id = "78d380fd-1b69-4e3c-8bc0-4f57737f7d5f"
            };

            // Receptor ESCUELA KEMPER URGATE
            var recipient = new InvoiceRecipient
            {
                Id = "bef56254-0892-4558-95c3-f9c8729e4b0e"
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    Id = "310301b3-1ae9-441b-b463-51a8f9ca8ba2",
                    Quantity = 1,
                    Discount = 10.85m,
                },
            };

            // Crear la factura 
            var invoice = new Invoice
            {
                VersionCode = "4.0",
                Series = "SDK-F",
                Date = DateTime.Now,
                PaymentFormCode = "01",
                CurrencyCode = "MXN",
                TypeCode = "I",
                ExpeditionZipCode = "42501",
                Issuer = issuer,
                Recipient = recipient,
                Items = items,
                PaymentMethodCode = "PUE",
            };


            // Timbrar la factura
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(invoice);


            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);

                Console.WriteLine($@"Factura {apiResponse.Data.Number} creada");
            }
            else
            {
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }


            if (apiResponse.Succeeded)
                return Ok(apiResponse);

            return BadRequest(apiResponse);
        }

        [HttpPost("factura-con-precios-dinamicos")]
        public async Task<IActionResult> CrearFacturaConPreciosDinamicos()
        {
           

            // Emisor KARLA FUENTE NOLASCO
            var issuer = new InvoiceIssuer
            {
                Id = "3f3478b4-60fd-459e-8bfc-f8239fc96257"
            };

            // Receptor ESCUELA KEMPER URGATE
            var recipient = new InvoiceRecipient
            {
                Id = "96b46762-d246-4a67-a562-510a25dbafa9"
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    Id = "114a4be5-fb65-40b2-a762-ff0c55c6ebfa",
                    Quantity = 1,
                    UnitPrice = 102.00m, // Precio dinámico
                },
            };

            // Crear la factura 
            var invoice = new Invoice
            {
                VersionCode = "4.0",
                Series = "SDK-F",
                Date = DateTime.Now,
                PaymentFormCode = "01",
                CurrencyCode = "MXN",
                TypeCode = "I",
                ExpeditionZipCode = "42501",
                Issuer = issuer,
                Recipient = recipient,
                Items = items,
                PaymentMethodCode = "PUE",
            };

            // Timbrar la factura
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(invoice);

            return Ok(apiResponse);
        }

        [HttpPost("nota-credito-con-precios-dinamicos")]
        public async Task<IActionResult> CrearNotaCreditoConPreciosDinamicos()
        {
           

            // Emisor KARLA FUENTE NOLASCO
            var issuer = new InvoiceIssuer
            {
                Id = "3f3478b4-60fd-459e-8bfc-f8239fc96257",
            };

            // Receptor ESCUELA KEMPER URGATE
            var recipient = new InvoiceRecipient
            {
                Id = "96b46762-d246-4a67-a562-510a25dbafa9"
            };

            // Agregar facturas relacionadas
            var relatedInvoices = new List<RelatedInvoice>()
            {
                new RelatedInvoice
                {
                    Uuid = "5FB2822E-396D-4725-8521-CDC4BDD20CCF",
                    RelationshipTypeCode = "01"
                }
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    Id = "114a4be5-fb65-40b2-a762-ff0c55c6ebfa",
                    Quantity = 0.5m, // 50% de descuento
                    UnitPrice = 101.00m, // Precio dinámico
                },
            };

            // Crear la factura de egreso (nota de crédito)
            var creditNote = new Invoice
            {
                VersionCode = "4.0",
                Series = "CN",
                Date = DateTime.Now,
                PaymentFormCode = "03",
                PaymentConditions = "Contado",
                CurrencyCode = "MXN",
                TypeCode = "E", // nota de crédito
                ExpeditionZipCode = "01160",
                Issuer = issuer,
                Recipient = recipient,
                Items = items,
                PaymentMethodCode = "PUE",
                ExchangeRate = 1,
                ExportCode = "01",
                RelatedInvoices = relatedInvoices
            };

            // Timbrar la factura de egreso
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(creditNote);

            return Ok(apiResponse);
        }

        // Crear nota credito por referencias (enviando solo lod ids de los objetos en la solicitud)
        [HttpPost("nota-credito-por-referencia")]
        public async Task<IActionResult> CrearNotaCreditoPorReferencia(CancellationToken ct = default)
        {
            /*Crear factura de egreso (nota de crédito) por referencias*/

            // Emisor KARLA FUENTE NOLASCO
            var issuer = new InvoiceIssuer
            {
                Id = "78d380fd-1b69-4e3c-8bc0-4f57737f7d5f",
            };

            // Receptor  ESCUELA KEMPER URGATE
            var recipient = new InvoiceRecipient
            {
                Id = "bef56254-0892-4558-95c3-f9c8729e4b0e"
            };

            // Agregar facturas relacionadas
            var relatedInvoices = new List<RelatedInvoice>()
            {
                new RelatedInvoice
                {
                    Uuid = "5FB2822E-396D-4725-8521-CDC4BDD20CCF",
                    RelationshipTypeCode = "01"
                }
                // otras facturas relacionadas aquí
            };

            // Crear una lista de productos o servicios de la factura
            var items = new List<InvoiceItem>()
            {
                new InvoiceItem
                {
                    Id = "310301b3-1ae9-441b-b463-51a8f9ca8ba2",
                    Quantity = 0.5m, // 50% de descuento
                },
                // otros productos o servicios aquí
            };

            // Crear la factura de egreso (nota de crédito)
            var creditNote = new Invoice
            {
                VersionCode = "4.0",
                Series = "CN",
                Date = DateTime.Now,
                PaymentFormCode = "03",
                PaymentConditions = "Contado",
                CurrencyCode = "MXN",
                TypeCode = "E", // nota de crédito
                ExpeditionZipCode = "01160",
                Issuer = issuer,
                Recipient = recipient,
                Items = items,
                PaymentMethodCode = "PUE",
                ExchangeRate = 1,
                ExportCode = "01",
                RelatedInvoices = relatedInvoices
            };

            // Timbrar la factura de egreso
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(creditNote);

            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);


                Console.WriteLine($@"Nota de Crédito {apiResponse.Data.Number} creada");
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
            }

            if (apiResponse.Succeeded)
                return Ok(apiResponse);

            return BadRequest(apiResponse);
        }


        // Crear complemento pago por referencias (enviando solo lod ids de los objetos en la solicitud)
        [HttpPost("complemento-pago-por-referencia")]
        public async Task<IActionResult> CrearComplementoPagoPorReferencia(CancellationToken ct = default)
        {
            // Emisor KARLA FUENTE NOLASCO
            var issuer = new InvoiceIssuer
            {
                Id = "78d380fd-1b69-4e3c-8bc0-4f57737f7d5f",
            };

            // Receptor  ESCUELA KEMPER URGATE
            var recipient = new InvoiceRecipient
            {
                Id = "bef56254-0892-4558-95c3-f9c8729e4b0e"
            };


            // pagos
            var payments = new List<InvoicePayment>()
            {
                //pago
                new InvoicePayment
                {
                    PaymentDate = DateTime.Now,
                    PaymentFormCode = "28",
                    CurrencyCode = "MXN",
                    Amount = 28599.99m,
                    SourceBankTin = "BSM970519DU8",
                    SourceBankAccount = "1234567891012131",
                    TargetBankTin = "BBA830831LJ2",
                    TargetBankAccount = "1234567890",
                    PaidInvoices = new List<PaidInvoice>
                    {
                        // factura pagada
                        new PaidInvoice
                        {
                            Uuid = "5C7B0622-01B4-4EB8-96D0-E0DEBD89FF0F",
                            Series = "F",
                            Number = "1501",
                            CurrencyCode = "MXN",
                            PartialityNumber = 1,
                            SubTotal = 10000m,
                            PreviousBalance = 9533.33m,
                            PaymentAmount = 9533.33m,
                            RemainingBalance = 0m,
                            TaxObjectCode = "02",
                            PaidInvoiceTaxes = new List<PaidInvoiceTax>
                            {
                                // impuestos de la factura 
                                new PaidInvoiceTax
                                {
                                    TaxCode = "002",
                                    TaxTypeCode = "Tasa",
                                    TaxRate = 0.160000m,
                                    TaxFlagCode = "T"
                                },

                                // otros impuestos 
                            }
                        },
                        // otras facturas pagadas 
                    }
                }
            };


            // Crear la factura de pago
            var paymentInvoice = new Invoice
            {
                VersionCode = "4.0",
                Series = "CP",
                Date = DateTime.Now,
                ExchangeRate = 1,
                CurrencyCode = "XXX",
                TypeCode = "P", // P para pagos
                ExpeditionZipCode = "01160",
                Issuer = issuer,
                Recipient = recipient,
                Items = null,
                Payments = payments
            };

            // Timbrar la factura de pago
            var apiResponse = await _fiscalapiClient.Invoices.CreateAsync(paymentInvoice);

            if (apiResponse.Succeeded)
            {
                // Guardar el XML de la factura
                var xml = apiResponse.Data.Responses.FirstOrDefault()?.InvoiceBase64.DecodeFromBase64();
                await System.IO.File.WriteAllTextAsync($@"C:\facturas\{apiResponse.Data.Number}.xml", xml, ct);
                Console.WriteLine($@"Factura de Pago {apiResponse.Data.Number} creada exitosamente");
                return Ok(apiResponse);
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
                return BadRequest(apiResponse);
            }
        }


        // Cancelar factura por referencias (enviando solo lod ids de los objetos en la solicitud)
        [HttpPost("cancelar-factura-por-referencia")]
        public async Task<IActionResult> CancelarFacturaPorReferencias(CancellationToken ct = default)
        {
            // request model
            var request = new CancelInvoiceRequest()
            {
                Id = "630a41d1-00ce-4750-8f8c-7b36786134d0", // Id factura en fiscalapi
                CancellationReasonCode = "01",
                ReplacementUuid = "de841944-bd4f-4bb8-adfe-2a2282787c62",
            };

            // Cancelar la factura
            var apiResponse = await _fiscalapiClient.Invoices.CancelAsync(request);

            // Check response
            if (apiResponse.Succeeded)
            {
                Console.WriteLine("Factura Cancelada.");
                return Ok(apiResponse);
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
                return BadRequest(apiResponse);
            }
        }


        // Enviar factura por correo por referencias (enviando solo lod ids de los objetos en la solicitud)
        [HttpPost("enviar-factura-email-por-referencia")]
        public async Task<IActionResult> EnviarFacturaEmailPorReferencia(CancellationToken ct = default)
        {
            /* Enviar factura por email por referencia*/

            // request model (personaliza el pdf de la factura)
            var request = new SendInvoiceRequest
            {
                InvoiceId = "1152fd7f-9470-4247-96e7-f1b3a28fdf6c",
                ToEmail = "mendoza.git@gmail.com", // email1@dominio;email2@dominio
            };

            // Send request
            var apiResponse = await _fiscalapiClient.Invoices.SendAsync(request);

            // Check response
            if (apiResponse.Succeeded)
            {
                Console.WriteLine("Factura enviada.");
                return Ok(apiResponse);
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
                return BadRequest(apiResponse);
            }
        }


        // Generar PDF por referencias (enviando solo lod ids de los objetos en la solicitud)

        [HttpPost("generar-pdf-por-referencia")]
        public async Task<IActionResult> GenerarPdfPorReferencia(CancellationToken ct = default)
        {
            //Generar PDF por referencias


            // request model (utiliza las reglas de reporte https://docs.fiscalapi.com/report-rules-info)
            var request = new CreatePdfRequest
            {
                InvoiceId = "51a3a6d3-b617-4a6b-8a67-cbfd654c7e90",
            };

            // Send request
            var apiResponse = await _fiscalapiClient.Invoices.GetPdfAsync(request);

            // Check response
            if (apiResponse.Succeeded)
            {
                var pdfBytes = Convert.FromBase64String(apiResponse.Data.Base64File);

                await System.IO.File.WriteAllBytesAsync(@"C:\facturas\MyFile.PDF", pdfBytes, ct);
                Console.WriteLine(@"PDF generado.");
                return Ok(apiResponse);
            }
            else
            {
                Console.WriteLine($@"HttpStatusCode: {apiResponse.HttpStatusCode}");
                Console.WriteLine(apiResponse.Message);
                Console.WriteLine(apiResponse.Details);
                return BadRequest(apiResponse);
            }
        }


        // Obtener factura por Id
        [HttpGet("{id}")]
        public async Task<IActionResult> Get(string id)
        {
            // Obtener factura por Id (true para expandir los objetos relacionados)
            var apiResponse = await _fiscalapiClient.Invoices.GetByIdAsync(id, true);

            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        } // Obtener factura por Id


        //Obtener status de factura por Id
        [HttpPost("obtener-status/{id}")]
        public async Task<IActionResult> Status(string id)
        {
            var requestModel = new InvoiceStatusRequest
            {
                Id = id,
            };

            var apiResponse = await _fiscalapiClient.Invoices.GetStatusAsync(requestModel);

            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        } // Obtener factura por Id


        // Obtener xml de factura por Id
        [HttpGet("{id}/xml")]
        public async Task<IActionResult> GetXml(string id)
        {
            // Obtener xml de factura por Id

            var apiResponse = await _fiscalapiClient.Invoices.GetXmlAsync(id);

            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }

        // Obtener lista paginada de facturas
        [HttpGet]
        public async Task<IActionResult> GetList([FromQuery] int pageNumber = 1, int pageSize = 2)
        {
            // Obtener lista paginada de facturas


            // envía la petición (pageNumber=1, pageSize=2)
            var apiResponse = await _fiscalapiClient.Invoices.GetListAsync(pageNumber, pageSize);

            if (apiResponse.Succeeded)
                return Ok(apiResponse.Data);

            return BadRequest(apiResponse);
        }
    }
}
